"""Run parameter sweeps for the Field Service Mesa model."""
...
def run_experiment(config: dict[str, Any], output_dir: Path | str) -> dict[str, Any]:
    ...
    for params in parameter_sets:
        for seed in seeds:
            model = FieldServiceModel(seed=seed, **params)
            for _ in range(steps):
                model.step()
            snapshot = model.snapshot()
            row = {"seed": seed, "steps": steps, **params, **snapshot}
            rows.append(row)
    ...
    for row in rows:
        if sweep_parameters:
            key = ", ".join(f"{name}={row[name]}" for name in sweep_parameters)
        else:
            key = "baseline"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the Field Service Mesa experiment.")
    parser.add_argument("--config", type=Path, default=Path("examples/field_service_mesa/experiment.json"))
    parser.add_argument("--output-dir", type=Path, default=Path("outputs/field_service"))
