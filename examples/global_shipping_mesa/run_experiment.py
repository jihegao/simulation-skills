"""Run parameter sweeps for the global shipping Mesa/SimPy model."""
...
def run_experiment(config: dict[str, Any], output_dir: Path | str) -> dict[str, Any]:
    ...
    for params in parameter_sets:
        for seed in seeds:
            model = GlobalShippingModel(seed=seed, **params)
    ...
    summary = {
        "model": "Global Shipping Mesa/SimPy",
        "steps": steps,
        "seeds": seeds,
        "run_count": len(rows),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the global shipping Mesa/SimPy experiment.")
    parser.add_argument("--config", type=Path, default=Path("examples/global_shipping_mesa/experiment.json"))
    parser.add_argument("--output-dir", type=Path, default=Path("outputs/global_shipping"))
