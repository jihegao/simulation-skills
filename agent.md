...
- `simulation_skills_contracts`：权威发布 `simulation.model_spec`、
  ...
  以及独立安装、只支持 `warehouse-queueing` 的真实 SimPy DES adapter。
...
- 不把 `simulation-skills-fake-adapter` 描述为真实 warehouse runtime、模型有效性证据或
  Workstream 1 实现。
- 不把 `simulation-skills-warehouse-des-adapter` 的单次队列执行描述为已校准仓库模型、
  comparison、敏感性分析、因果证明或完整 Workstream 1/2 闭环。
